...
resolve_build() {
    ...
    DOWNLOAD_URL=$(echo "$resp" | grep -o '"browser_download_url":"[^"]*"' | grep '\.jar' | sed 's/.*"browser_download_url":"\([^"]*\)".*/\1/' | tr '\\' '/')
...
maybe_update_vialimbo() {
    ...
    local url; url=$(echo "$resp" | grep -o '"browser_download_url":"[^"]*"' | grep '\.jar' | sed 's/.*"browser_download_url":"\([^"]*\)".*/\1/' | tr '\\' '/')
